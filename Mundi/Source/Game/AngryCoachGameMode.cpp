...
void AAngryCoachGameMode::PostLogin(APlayerController* NewPlayer)
{
	...
	if (AActor* P1Actor = GWorld->SpawnPrefabActor(P1PrefabPath))
	{
		...
		if (Player1)
		{
			Player1->SetActorLocation(FVector(0, -15, 0));
	...
	if (AActor* P2Actor = GWorld->SpawnPrefabActor(P2PrefabPath))
	{
		...
		if (Player2)
		{
			...
			Player2->SetActorLocation(FVector(0, 15, 0));
...
void AAngryCoachGameMode::ResetPlayersHP()
{
	...
	{
		Player1->Revive();
		Player1->SetActorLocation(FVector(0, -15, 0));  // 초기 위치로 리셋
		Player1->SetActorRotation(FVector(0, 0, 90));   // 회전 리셋 (Player2를 향함)
	}
	if (Player2)
	{
		Player2->Revive();
		Player2->SetActorLocation(FVector(0, 15, 0));   // 초기 위치로 리셋
		Player2->SetActorRotation(FVector(0, 0, -90));  // 회전 리셋 (Player1을 향함)
	}
}
